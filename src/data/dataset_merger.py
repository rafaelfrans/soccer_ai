...
def merge_datasets(
    ...
    primary_images: dict[str, list[Path]] = {}
    primary_labels: dict[str, dict[str, Path]] = {}
    ...
    ball_only_images: list[Path] = []
    ball_only_labels: dict[str, list[str]] = {}
    ...
    if len(ball_only_images) > max_ball_only:
        ...
        # Update labels dict
        ball_only_labels = {
            img.name: ball_only_labels.get(img.name, []) for img in ball_only_images if img.name in ball_only_labels
        }
    ...
    for split in ["train", "val", "test"]:
        for img_path in primary_images[split]:
            ...
            if img_path.name in primary_labels[split]:
                shutil.copy2(
                    primary_labels[split][img_path.name], out_dir / split / "labels" / (img_path.stem + ".txt")
                )
    ...
    for i, img_path in enumerate(ball_only_list):
        ...
        if img_path.name in ball_only_labels:
            label_path = out_dir / split / "labels" / (img_path.stem + ".txt")
    ...
    print(f"✅ Merged dataset saved to: {out_dir}")
    print("📊 Statistics:")
...
def _extract_ball_annotations(label_path: Path, ball_class_id: int) -> list[str]:
    """
